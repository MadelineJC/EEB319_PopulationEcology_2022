...
This year we don't have access to our lab cultures so we will use data collected by former students. We typically prepare several *Tenebrio* cultures and groups of students count the number of insects in each life stage. This year let's use data from a single culture: # larvae = 11563, # pupae = 221, # adults = 625. Use these data to build a vector $N_t$ with the abundance of each life stage at time *t*:
```{r, eval=TRUE}
# N_t <- c(11563,221,625)
...
***Table 1:** Life history parameters for Tenebrio molitor.*

![](Lab3_Images/Lab3_Table1.png "LHT"){height=125px width=500px} 

*References: Canadian Grain Commission. http://www.grainscanada.gc.ca; Dick, J. 1937. Annals of Applied Biology 4: 762-796.; Greenberg, S. & A. Ar 1996. Journal of Insect Physiology 42: 991-996.; Hill, D.S. 2002. Pests of stored foodstuffs and their control. Kluwer Academic Publishers, Dordrecht.*  
...
Since we assume the finite growth rate is constant for all life stages (λ), we can estimate the probability of surviving through the larval stage ($s_l$) by rearranging equation 1 to isolate $λ_l$ and by replacing again $N_a$ and $N_l$ by $l_a$ and $l_l$:  

\begin{align}
    λ_l = \frac{N_{l(t+1)}}{N_{l(t)}} = P_l + \frac{F_a N_{a(t)}}{N_{l(t)}} = s_l (1-g_l) + F_a l_a \\
\end{align}  

and rearranging equation (2) to isolate $λ_p$ (equation (6)), and setting $λ_l$ = $λ_p$:  
...
Now, let's write all this in R code:
```{r, eval=TRUE}
...
Let’s first explore the predictions for our mealworm beetle population. You have a matrix **M** with the estimated vital rates for this species and a vector **$N_t$** with the observed stage structure of our lab population. You now need to multiply repeatedly the stage-based transition matrix to the stage structure of your population at different times in the future (time steps t+1, t+2, … t+72). 
...
```{r, eval=TRUE}
# nTimeSteps <-    # Specify the number of time steps we'd like to consider
# M    # Our transition matrix (make sure it is right)
...
We can plot the trajectory of our population over time using the **plot()** function. Remember that these predictions are just as good as the data used to build the model. Think carefully about the assumptions you made and uncertainties (e.g., counts from subsamples of our lab population, the range of life history parameters in Table 1, other assumptions made as estimated the parameters of our transition matrix) as you interpret the predicted dynamics. 
...
\begin{align}
    \text{(9) } λ = \frac{N_{t+1}}{N_{t}} = e^{r⋅t}\\
\end{align}  
...
\begin{align} 
    \text{(10) } r = \frac{ln(\frac{N_{t+1}}{N_{t}})}{t}\\
\end{align}  
...
Calculate how quickly our population doubles in size and how how long it will take to increase 5 times. **Make sure $t_{2x}$ and $t_{5x}$ are expressed in days**:
```{r, eval=TRUE}
# t_2x <- log(2) / r  
# t_5x <- .....
```
...
Repeat this exercise for your two managed populations to convince yourself that any set of constant vital rates leads to a *Stable Stage Structure*, meaning that the proportions of individuals in each stage remain constant over time regardless of initial abundances. Prepare graphs that clearly demonstrate this principle (e.g. one graph for each population). **Think carefully what you need to clearly demonstrate this principle.**  
