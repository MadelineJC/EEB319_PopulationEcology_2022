...
## Lab Goals  
1) Estimate the growth rate of a Yellow mealworm beetle population (we will use data from former students since this year we don't have access to the lab cultures)
...
Based on the numbered equations above, we can estimate and calculate all parameters we need to run the Stage-Structured Projection Matrix Model. First, you need to type out the names of your parameters, and assign them the values you estimated. Then, you can construct your stage-based transition matrix (**M**) using R's **matrix()** function. I've included an example in the code chunk below to build a 3x3 matrix. 
```{r, eval=TRUE}
# Notice how R builds matrices, and adapt this to build your M matrix
...
Then write your vector $N_t$, with the number of individuals in each life stage that were found in our lab population a few years ago ($N_l$=11563, $N_p$=221, $N_a$=625). 

```{r, eval=TRUE}
# Assign the number of individuals counted in each life stage and build the vector here:
# ...
# N_t <- matrix()
```
...
(1) Predict dynamics of the lab population.  
...
We can do all this work in R using matrix multiplication! Let's say our initial abundances, the abundances of each life stages at $t_0$, are all 1. Let's make a vector to represent that (P.S. Remember to remove the "#"!):  
```{r, eval=TRUE}
# N_0 <- c()
```

Now, to get our population abundances for the next time step, $t_1$, we just need to multiply our transition matrix **M** by that vector of initial abundances. The output of this operation will denote our population abundances at the next time step.  
```{r, eval=TRUE}
# N_1 <- M %*% N_0
# N_1
```
...
# N_0 <- c() # Initial population abundances
# 
...
```{r, eval=TRUE}
# plot(1,1,pch="",ylim=c(0,max(N_projected)),xlim=c(0,nTimeSteps+1),xlab="Years",ylab="Abundance") # Set up plot
...
<p style="color: red;">**Question 1: What would have happened if we had just started the mealworm population with one larva, one pupa and one adult? Repeat the population projection with this $N_0$ vector and compare the two graphs.** </p> 

Calculate the Finite *per capita* Population Growth Rate (λ) at each time step. This rate is the total number of individuals in the population (all stages included) one time step in the future ($N_{(t+1)}$) divided by the total number of individuals at the current time ($N_{(t)}$):  
...
Plot λ and *r* over time. 

<p style="color: red;">**Question 2: What happens to $λ_t$ and *r* in the long run?** </p> 

We can find the dominant eigenvalue (λ) of the transition matrix using a function from the popbio package, **eigen.analysis()**.  
```{r, eval=TRUE}
# dom_eigen <- eigen.analysis(M, zero = FALSE)
```


The R package [**"popdemo"**](https://cran.r-project.org/web/packages/popdemo/popdemo.pdf) can also help you out with a lot of these calculations. Install and load popdemo, and explore its **eigs()** function, which can give the dominant eigenvalue (asymptotic population growth), the dominant right eigenvector (stage distribution), and the dominant left eigenvector (reproductive value) of a transition matrix.  
```{r, eval=TRUE}
# Install
...
* Calculate the long-term Finite per capita Growth Rate (λ) and the Instantaneous per capita Growth Rate (r) for our lab population. What does this mean in terms of population dynamics (i.e., is the population stable, increasing or decreasing)? How long would it take this population of beetles to double in size? (5 marks).  
