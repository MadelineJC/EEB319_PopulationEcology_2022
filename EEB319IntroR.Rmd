...
In R, functions are commands you use to tell R to perform some task. The good news is, you've already used a couple functions ("read.csv()", "class()"). To use functions, you type out the function's name and open brackets, "()". Inside your brackets, you'll include your arguments, which tell R where and how you're using a function. R has a lot of built-in functions, but we can find new functions in packages, and even write our own (but we'll talk more about that later).  

**Let's take a look at our data!** You can view your data set in its entirety by using the function, **View()**:  
```{r, eval=FALSE}
View(InfecData)
```

Do your data look similar to how they looked in Excel? You should have 501 rows and 3 columns (time, parasite abundance, and host immune cell abundace). Next, let's use some more functions to explore our data!  
