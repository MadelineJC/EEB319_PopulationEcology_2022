...
Now, to simulate this model forward in time, open the Lab 1 Excel sheet; come back here when you've finished simulating your data! **NOTE: Values for the parameter "r" can range between 0.1 and 2.0. Play around with a few different values to see how your simulation changes!**  
...
### Importing Data  
Remember to make sure your data are stored in a .csv file, in your *working directory* (above). **You can import your data using read.csv():**  

Make sure to give your data set a name so you can call on it later; here we're calling our data set "InfecData", given that our data chronicle an infection. We use "**<-**" ("option" + "-")  as an assignment operator: it assigns a value to an object. Here, the value is our data set (.csv file), and the object is "InfecData".  
```{r, eval=FALSE}
# NOTE 1: Remember to change eval=TRUE!

# NOTE 2: We can put a "#" at the beginning of a line of code if we don't want R to run that line. These lines are called "comments" and can be used to leave yourself notes explaining what your code does, why you chose to do something a certain way, etc.! It's good practice to provide clear comments so anybody (e.g. your TA) can pick up your code and understand what you're trying to do.

# NOTE 3: To run a line or chunk of code in your editor, use "command + Enter" on Mac, and "Ctrl + Enter" on Windows. If you're working in your console (bottom-left window in default layout), you can just use "Enter".

InfecData <- read.csv("PathNameHere.csv") # NOTE: that you need to include the entire pathname for your .csv within the quotation marks.
```
...
which(InfecData$Parasites == max(InfecData$Parasites)) # Reads: Where in the "Parasites" column of the the data set (InfecData$Parasites) is the data point equal to (==) the maximum abundance of parasites (max(InfecData$Parasites)); shows us when the parasite population peaks.
...
Let's think about our Within-Host Disease Dynamics example from ealier... We simulated our model forward in time using Excel, but we can do the same thing in R. We can use a for loop to calculate future population abundances, and if... then statements to think about and account for population thresholds (when populations fall below zero, and when the parasite population reaches a lethal abundance). The output of this code chunk should resemble the plot you generated earlier!  
...
Sim_Func <- function(Para_Last, Immune_Last){
  ...
  Para_Next = (1 + r*ts -k*Immune_Last*ts)*Para_Last
  Immune_Next = (1 + p*(Para_Last/(Para_Last + o))*ts)*Immune_Last
  
  Para_Last <- Para_Next
  Immune_Last <- Immune_Next
  
  # Add an if... then statement to prevent the population from going negative...
  if (Para_Last < 0){
    Para_Last <- 0; Immune_Next <- 0
  } # If the parasite population abundance falls under 0, set abundance to 0
  
  if (Para_Last >= 10^9){
    Immune_Next <- 0
  } # If the parasite population abundance exceeds 10^9, the host dies; set immune cell population abundance to 0
  
  if (Immune_Last < 0){
    Immune_Last <- 0
  } # If the immune cell population abundance falls under 0, set abundance to 0
